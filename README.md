...
- 管理画面の「登録一覧」タブに、デモ版だけの**一括生成**があります（件数を指定して実行）
  - **一括で追加** … `Title 1` / `Name 1` のような連番の登録データを、いまの一覧に足します
  - **一括で作成** … いまの登録をすべて置き換えて作り直します
  - 種別は定義順に一巡ずつ、発表日は会期に均等に割り当てます（会期が未設定なら「未定」）。
    最大500件まで。タイムテーブルタブを開くと、下書きの無い発表日はこの登録から自動で組み立てられます
