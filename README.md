...
管理画面の**設定**で**会場**を入れていると、イベント名の下に「会場 ○○」と表示されます。
...
- 見出しの下に**会期**と**会場**（設定していれば）が並びます。イベント名と合わせて、印刷 / PDF にも出ます。
- 会期が複数日ある場合は**発表日**を切り替えて表示でき、「すべて」で全日をまとめて表示できます。
  この状態で印刷 / PDF にすると、1日ごとにページが分かれます。
- 参加登録を**受付中**のあいだは、右上に**発表申込**（参加登録ページへのリンク）が出ます
  （参加登録ページ側の「プログラム」と対になる導線です）。受付を止めると消えます。
  タイムテーブルが非公開のあいだも、受付中なら出ます。
...
管理画面は開いた直後は**閲覧モード**で、何も変更できません。
画面上部の「**編集を開始**」を押すと、そのブラウザが**編集権**を持ちます。
編集権を持っているあいだ、ほかの人の管理画面には「◯◯が編集中です」と出て閲覧のみになります。
閲覧モードでも、タブの切り替え・再読み込み・CSVエクスポート・印刷 / PDF に加えて、
タイムテーブルの**発表日の切り替え**ができます（見るだけなので、切り替えても保存はされません）。

管理者パスワードは1つしか無いので、誰が編集しているかを区別する材料がサーバーにはありません。
そのため「編集を開始」のときに名前を聞きます（空欄なら「他の管理者」と表示されます）。

- 編集が終わったら「**編集を終了**」を押してください。すぐにほかの人が編集できるようになります
- 押し忘れてタブを閉じても、**2分**（[`EDIT_LEASE_SECONDS`](#環境変数)）で自動的に解放されます
- 急ぐ場合は「**編集を引き継ぐ**」で強制的に奪えます。相手の画面は閲覧モードに戻ります
  （相手がまだ保存していない変更は失われることがあります）
- 「編集を開始」を押すと画面を読み込み直します。待っているあいだのほかの人の変更を、
  古い手元の内容で上書きしてしまわないようにするためです

制限はサーバー側でも掛かっていて、編集権を持たないリクエストは更新系のAPIすべてで拒否されます。
なお**参加登録ページからの申込は編集権とは無関係**で、いつでも受け付けます。
...
- 操作パネルを隠して出力する**印刷 / PDF**レイアウト
  （紙にはイベント名・会期・会場が出ます。登録件数・受付状態・公開状態などの管理用の表示は出ません）
- 編集内容は**下書きとして自動保存**され、次に開いたときに復元されます

**設定**
受付の開始／停止、参加登録キー、イベント名、会期、会場、参加登録ページに表示する案内文、
...
**会場**は任意の1行です（200文字まで）。指定すると参加登録ページと公開ページの見出しの下に
「会場 ○○」と出ます。空欄なら何も表示しません。会場が複数ある・部屋が発表ごとに違うといった場合は、
**案内文**やタイムテーブルの**特別**枠のほうが向いています。
...
| `EDIT_LEASE_SECONDS` | `120` | [編集権](#管理画面)が、編集中の画面から連絡が来なくなってから自動で解放されるまでの秒数（最短30秒） |
...
- 更新系のAPIはさらに**編集権**（[管理画面](#管理画面)）を要求します。持っていないリクエストは `423` で拒否されるので、
  2つのタブや2人の管理者が同じ下書きを上書きし合うことはありません。ログアウトすると編集権も手放されます。
...
- 種別・会期・会場などの**設定だけ**が入った状態で始まります（**登録は0件**）。参加登録キーは `demo` です
- 登録データは、参加登録ページから申し込むか、下記の一括生成で作ります
- 公開ページ（`program.html`）も試せます。デモでは公開フラグを最初から立ててあります
  （登録もタイムテーブルの下書きも無いあいだは「準備中」と表示されます）
- 画面上部のバーから「デモデータを初期化」で初期状態に戻せます
- 管理画面の「登録一覧」タブに、デモ版だけの**一括生成**があります（種別と件数を指定して実行）
  - **一括追加** … `Title 1` / `Name 1` のような連番の登録データを、いまの一覧に足します
  - **全て削除** … いまの登録をすべて消します（設定とタイムテーブルの下書きは残ります）
  - 種別は選んだものを全件に、「順番に」なら定義順で一巡ずつ割り当てます。
    発表日は会期に均等に割り当てます（会期が未設定なら「未定」）。最大500件まで
  - タイムテーブルタブを開くと、下書きの無い発表日はこの登録から自動で組み立てられます
